%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This is the main file of the Msc thesis template.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Author:   René Widmer
%           Institute for Surgical Technology and Biomechanics ISTB
%           University of Bern
%           rene.widmer@istb.unibe.ch
%
% Date:     10/28/2009
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\pdfminorversion=6
\documentclass[a4paper,10pt,openright]{unibe-msc}
% Some regulary used packages
\usepackage{etex}
\usepackage[T1]{fontenc}
\usepackage{textcomp}
\usepackage{bm}
\usepackage[utf8]{inputenc}
\usepackage[pdftex,final]{graphicx}
\usepackage{amssymb}
\usepackage{amsmath}                    
\usepackage{amsfonts}
\usepackage{mathtools}
\usepackage{ulem}
\usepackage[english]{babel}
\usepackage{url}
\usepackage{color}
\usepackage{unibe-msc}
\usepackage[caption=off]{subfig}
\usepackage{hyperref}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Luca Sahli's setup
% *******************************************************************
% *******************************************************************
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Luca Sahli's packages
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\usepackage{float} % better positioning of images
\usepackage{verbatim} % comment environment (that skips everything between \begin{comment} and \end{comment})
\usepackage{smartdiagram} % create ‘smart’ diagrams from lists of items, for simple documents and for presentations. 
% \usepackage{subcaption} % provides a means of using facilities analagous to those of the caption package, when writing captions for subfigures and the like
\usepackage{svg} % to include svg figures with \includesvg{file.svg}
\usepackage{gensymb} % to use \degree for the ° symbol
\usepackage{multirow} % Create tabular cells spanning multiple rows
\usepackage{booktabs} % The package enhances the quality of tables in LATEX, providing extra commands
\usepackage{makecell} % 
\usepackage{titlesec}
\usepackage{pdfpages} % to include pdf pages
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Luca Sahli's commands
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand*{\addheight}[2][.5ex]{%
  \raisebox{0pt}[\dimexpr\height+(#1)\relax]{#2}%
}
% \newcommand*\mc[1]{\multicolumn{3}{c}{#1}}
\makeatletter
\renewcommand\paragraph{\@startsection{paragraph}{4}{\z@}%
                                     {-3.25ex\@plus -1ex \@minus -.2ex}%
                                     {1.5ex \@plus .2ex}%
                                     {\normalsize\bfseries}}
\renewcommand\subparagraph{\@startsection{subparagraph}{5}{\z@}%
                                     {-3.8ex\@plus -2ex \@minus -.2ex}%
                                     {1.5ex \@plus .2ex}%
                                     {\small\bfseries}}

\makeatother
% *******************************************************************
% *******************************************************************



% Default graphics path
\graphicspath{{Images/}{Logos/}{images/}}

% Document metadata
\unibelogo{ub_16pt_192}  % Don't change this
\htilogo{TI_e} % Don't change this
\faculty{Faculty of Medicine} % Don't change this
\discipline{Biomedical Engineering} % Don't change this
\subtitle{Master of Science Thesis} % Don't change this
\title{Intraoperative 3D ultrasound-based planning for surgical resection of
  liver tumors}
\author{Luca Sahli}
\origin{Wohlen bei Bern, Switzerland}
\date{\today}
\supervisor{MSc Iwan Paolucci and Prof. Dr.-Ing. Stefan Weber}
\affiliation{ARTORG Center for Biomedical Engineering Research, University of Bern}

\examiner{Prof. Dr.-Ing. Stefan Weber and MSc Iwan Paolucci}
\place{Bern}
\frontsignature{\theplace, October 2018}
\date{October 31\textsuperscript{th} 2018}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Hyperref setup
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Appearance of hyper links
\definecolor{lc}{rgb}{0,0,0}
\hypersetup{colorlinks=true, breaklinks=true, linkcolor=lc, 
                  menucolor=lc, urlcolor=lc, anchorcolor=lc,
                  citecolor=lc, filecolor=lc,
                  pdftitle=\thetitle,
                  pdfauthor=\theauthor,
                  pdfsubject=Master Thesis,
                  pdfkeywords={Keyword 1, Keyword 2, Keyword 3},
                  pdfpagelayout=SinglePage}
                  
% Bibliography style file
\bibliographystyle{ieee}
\begin{document}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Titlepage, abstract table of contents etc.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\frontmatter
\maketitle
\input{Abstract}
\clearpage
\input{Acknowledgments}
\clearpage
\input{Conformity}
\cleardoublepage
\tableofcontents

\mainmatter

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Main text body
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\include{Introduction}
\include{ProblemStatement}
\include{StateOfTheArt}
\include{Concept}
\include{Implementation}
\include{Experiments}
\include{Conclusion}
\include{Outlook}

% Uncomment this if you'd like to suppress the numbering of the
% appendices.
%\backmatter

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Bibliography
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Force every reference to show up (demo)
\nocite{*}
\bibliography{References}

% {
%    \vskip1em
%    \noindent\textit{etc.}
% } % Remove this block.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Appendices
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\part*{Appendices}
\begin{appendix}
	\include{Appendix_1}
\end{appendix}

\end{document}

\endinput
%%% Local Variables:
%%% TeX-master: t
%%% End: